<?php
    ...
    foreach ($decode['mhs'] as $row) {
        $sql = mysqli_query($conn, "SELECT a.*, b.* FROM setoran_registrasi a, mahasiswa b 
        					WHERE a.nim = b.nim 
        					AND a.no_pendaftaran = '$row[no_pendaftaran]'");
        $fd = mysqli_num_rows($sql);
        ...
        if($fd > 0) 
        {
            ...
            mysqli_query($conn, "UPDATE mahasiswa SET kabupaten = '$row[kabupaten]',
